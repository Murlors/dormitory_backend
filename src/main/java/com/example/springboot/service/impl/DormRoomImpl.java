...
public class DormRoomImpl extends ServiceImpl<DormRoomMapper, DormRoom> implements DormRoomService {
     ...
     */
    @Override
    public int adjustRoomUpdate(AdjustRoomInfo adjustRoomInfo) {
        //调宿人
        String username = adjustRoomInfo.getUsername();
        //当前房间号
        int currentRoomId = adjustRoomInfo.getCurrentRoomId();
        //当前床位名称
        String currentBedName = JudgeBedName.getBedName(adjustRoomInfo.getCurrentBedId());
        //目标房间号
        int towardsRoomId = adjustRoomInfo.getTowardsRoomId();
        //目标目标房间号
        String towardsBedName = JudgeBedName.getBedName(adjustRoomInfo.getTowardsBedId());
        QueryWrapper qw = new QueryWrapper();
